...
class SlpRESTControllerLib {
  ...
  async address (ctx) {
    try {
      ...
      const result = await _this.useCases.slp.getAddress(address)
  ...
  /**
   * @api {post} /slp/tx Get TX Data
   * @apiPermission public
   * @apiName TX
   * @apiGroup REST SLP
   * @apiDescription Get transaction data, hydrated with token information.
   *
   * @apiExample Example usage:
  ...
  async tx (ctx) {
    try {
      ...
      const result = await _this.useCases.slp.getTx(txid)
  ...
  /**
   * @api {post} /slp/token Token Stats
   * @apiPermission public
   * @apiName Token
   * @apiGroup REST SLP
  ...
  async token (ctx) {
    try {
      ...
      const result = await _this.useCases.slp.getToken(tokenId, withTxHistory)
  ...
  async status (ctx) {
    try {
      const status = await _this.useCases.slp.getStatus()
...
// module.exports = SlpRESTControllerLib
